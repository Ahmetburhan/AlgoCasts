...
// Initialize a start index of 0 and an end of index length - 1
// Perform a while loop until end is less than start
// Compute the mid index by taking the Math.floor of the average of start + end
// If the target is found at mid, return mid
// Otherwise, check which side is sorted by comparing the start and end values to the mid.
// The target value should either exist in -between the sorted region or outside.
// Discard the unused half by updating the ‘end’ to mid - 1 or ‘start’ to mid+ 1
// If the while loop breaks without a matching value found, return -1
